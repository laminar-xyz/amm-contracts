...
import '../../v2/core/interfaces/ILaminarV2Callee.sol';
...
contract TokenValidator is ITokenValidator, ILaminarV2Callee, ImmutableState {
    ...
    function _validate(
        ...
        address pairAddress = LaminarV2Library.pairFor(this.factoryV2(), token, baseToken);
        ...
        (, bytes memory returnData) = address(pairAddress).call(abi.encodeWithSelector(ILaminarV2Pair.token0.selector));
        ...
        ILaminarV2Pair pair = ILaminarV2Pair(pairAddress);
    ...
    function laminarV2Call(
        address,
        uint256 amount0,
        uint256,
        bytes calldata data
    ) external view override {
        ILaminarV2Pair pair = ILaminarV2Pair(msg.sender);
